...
interface ProjectMemberCreateAndAssignFormProps {
	projectId: number;
	onSuccess?: () => void;
	onCancel?: () => void;
}

interface MemberCreateAndAssignInput {
	email: string;
	name: string;
	password?: string;
}
...
export function ProjectMemberCreateAndAssignForm({
	projectId,
	onSuccess,
	onCancel,
}: ProjectMemberCreateAndAssignFormProps) {
	...
	} = useForm<MemberCreateAndAssignInput>({
		defaultValues: {
			email: "",
			name: "",
			password: "",
	...
	const onSubmit = async (data: MemberCreateAndAssignInput) => {
		try {
			const isRandom = !data.password;
			const passwordToUse = data.password || generatePassword();

			// 1. Invite/Create member in organization
			const orgResult = (await inviteMemberMutation.mutateAsync({
				email: data.email,
				name: data.name,
				password: passwordToUse,
			})) as unknown as { success: boolean; userId?: number; message?: string };

			// 2. Assign the member to the project immediately
			if (orgResult && orgResult.userId) {
				await assignMemberMutation.mutateAsync({
					projectId,
					userId: orgResult.userId,
				});

				toast.success(`Assigned ${data.name} to the project!`);
			}
	...
	const isPending = inviteMemberMutation.isPending || assignMemberMutation.isPending;
			...
			<DialogContent className="max-w-md rounded-lg p-0 overflow-hidden max-h-[90vh] flex flex-col">
				<DialogHeader className="p-6 pb-2">
					...
					<DialogDescription className="text-xs text-muted-foreground font-light ml-1">
						The member has been added to your organization and assigned to the project.
		...
		<DialogContent className="max-w-md rounded-lg p-0 overflow-hidden max-h-[90vh] flex flex-col">
			<DialogHeader className="p-6 pb-2">
				...
				<DialogDescription className="text-xs text-muted-foreground font-light mt-1">
					Invite a new member to the organization workspace and instantly assign them to this project.
				</DialogDescription>
			</DialogHeader>

			<ScrollArea className="flex-1 min-h-0">
				<form
					id="project-member-create-assign-form"
				...
				>
					<div className="space-y-4">
						...
						{/* API Errors */}
						{(inviteMemberMutation.isError || assignMemberMutation.isError) && (
							<p className="text-[10px] font-bold text-destructive bg-destructive/10 border border-destructive/20 rounded-lg px-4 py-3 uppercase tracking-wide leading-relaxed">
								{inviteMemberMutation.error?.message || assignMemberMutation.error?.message || "An error occurred. Please try again."}
							</p>
						)}
					</div>
				</form>
			...
			<DialogFooter className="p-6 pt-2 border-t border-border flex gap-2 justify-end">
				...
				<Button
					type="submit"
					form="project-member-create-assign-form"
